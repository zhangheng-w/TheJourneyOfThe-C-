...
struct _Deque_iterator {
  typedef _Deque_iterator<_Tp, _Tp&, _Tp*>             iterator;       //iterator
  typedef _Deque_iterator<_Tp, const _Tp&, const _Tp*> const_iterator; //const_iterator
  static size_t _S_buffer_size() { return __deque_buf_size(sizeof(_Tp)); }

  //数据类型 别名 typedef
  typedef random_access_iterator_tag iterator_category;   //连续迭代器，外界假象
  typedef _Tp value_type; //数据类型
  typedef _Ptr pointer;   //指针
  typedef _Ref reference; //引用
  typedef size_t size_type;
  typedef ptrdiff_t difference_type;
  typedef _Tp** _Map_pointer;      //指向内存的指针
  ...
  _Tp* _M_first;//头元素指针
  _Tp* _M_last; //尾元素指针
  _Map_pointer _M_node;//指向内存当前位置，(是一个二维数组)

  //构造函数
  _Deque_iterator(_Tp* __x, _Map_pointer __y) 
    : _M_cur(__x), _M_first(*__y),
      _M_last(*__y + _S_buffer_size()), _M_node(__y) {}
  _Deque_iterator() : _M_cur(0), _M_first(0), _M_last(0), _M_node(0) {} //默认构造函数

  _Deque_iterator(const iterator& __x) //拷贝构造函数
...
protected:
  _Tp** _M_map;       //内存
  size_t _M_map_size; //长度
  iterator _M_start;  //头部指针
  iterator _M_finish; //尾部指针
...
// const_iterator  只能读数据，不能改写  函数重载-(返回类型或者参数不同)
...
//头尾元素
  ...
  reference back() {
    iterator __tmp = _M_finish; //尾地址前面一个元素
    --__tmp;
    return *__tmp;
  }
//不可改变头尾元素 const 函数后面----返回的元素不可改变
